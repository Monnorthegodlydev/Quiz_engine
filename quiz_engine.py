...
try:
    ...
    for i in range(1, len(sys.argv), 2):
        ...
        qadata[que_key] = sys.argv[i].replace("'", " ")
        qadata[ans_key] = sys.argv[i + 1].replace("'", " ")
        local_qnum += 1
...
try:
    ...
    while local_qnum <= len(sys.argv[1:]) // 2:
        ...
        print(quiz_ai_processor(usea, local_qnum, que_key, 80))
        local_qnum += 1
    print('The Quiz is Over, Have a Nice day!!')
